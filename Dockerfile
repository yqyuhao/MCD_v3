...
# Annovar 2017-07-17
WORKDIR $software/source
RUN git clone http://github.com/yqyuhao/MCD_v3.git && cd MCD_v3 && unzip annovar_2017-07-17.zip && cd annovar && cp *.pl $software/bin

# copy esssential files
WORKDIR $software/source
RUN cd MCD_v3 && cp fastq2stat.pl api_baopi-post.py MCD_baopi_simple_v3 $software/bin/ && cp T554V1.bed $software/target/
